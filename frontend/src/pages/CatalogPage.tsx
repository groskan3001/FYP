...
const CatalogPage = () => {
  ...
  return (
    <main className="container py-8 md:py-12" style={{ minHeight: '80vh' }}>
      <SectionHeading
        eyebrow="Catalog"
      ...
      />
      <div className="flex gap-md mb-6 max-w-xl" style={{ maxWidth: '600px' }}>
        <input
          type="search"
      ...
      <div className="flex flex-wrap gap-sm mb-8" aria-label="quick categories">
        {categories.map((category) => (
          <button
            key={category}
            type="button"
            className={`btn ${filters.category === category ? 'btn-primary' : 'btn-outline'}`}
            style={{ borderRadius: 'var(--radius-full)', padding: '0.25rem 1rem' }}
      ...
      {isError && <p className="text-body text-primary p-4 bg-surface-container rounded-md">Something went wrong while loading the catalog.</p>}
      {!isLoading && !isError && activeResults === 0 && <p className="text-body text-muted p-4 bg-surface-container rounded-md text-center">No products match your filters (yet).</p>}
      <div className="grid gap-lg" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))' }}>
        {data?.products.map((product) => (
          <ProductCard key={product._id} product={product} onAddToCart={handleAddToCart} />
        ))}
      </div>
      {addingToCart && <p className="text-body text-secondary mt-4">Adding to cart…</p>}
    </main>
  );
};
